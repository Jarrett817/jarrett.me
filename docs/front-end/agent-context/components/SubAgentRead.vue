  ...
  <div class="mt-3 text-left" data-reveal-interactive @click.stop>
    ...
    <div class="grid grid-cols-2 gap-2">
      ...
      >
        ...
        <div class="mt-0.5 text-[0.72rem] text-gray-500">{{ approach.tag }}</div>
      </button>
    </div>
    ...
    <div v-if="active === 'chunk'" class="mt-2 space-y-2">
      <div class="rounded-lg border border-gray-200 bg-gray-50 p-2.5 text-[0.72rem] text-gray-700">
        ...
        <div class="mt-2 flex items-center gap-1.5 text-[0.72rem] text-gray-500">
          <span class="rounded bg-amber-100 px-1.5 py-0.5 text-amber-800">缺点</span>
          大文件需要多轮调用，每轮内容都占 context window，容易触发压缩
        </div>
      </div>
      <div class="flex items-center justify-center gap-1 text-[0.72rem] text-gray-500">
    ...
    <div v-else class="mt-2 space-y-2">
      ...
      <div class="rounded-lg border border-[#dbe3f0] bg-white p-2.5">
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 rounded-lg border border-gray-200 bg-[#fafbfc] p-2 text-center">
            <div class="text-[0.72rem] font-bold text-gray-600">主 Agent</div>
            <div class="mt-1 text-[0.68rem] text-gray-500">保持精简上下文</div>
            <div class="mt-2 rounded bg-[#eef3fb] px-2 py-1 text-[0.68rem] text-[#3e66ae]">
              "分析这个 8000 行文件的架构"
            </div>
          </div>
          ...
          <!-- 箭头 -->
          <div
            class="flex flex-col items-center justify-center gap-0.5 pt-3 text-[0.8rem] text-gray-400"
          >
            <span>委派 →</span>
            <span>← 结论</span>
          </div>
          ...
          <div class="flex-1 rounded-lg border border-emerald-200 bg-emerald-50/30 p-2 text-center">
            <div class="text-[0.72rem] font-bold text-emerald-700">Sub-agent</div>
            <div class="mt-1 text-[0.68rem] text-gray-500">独立 context window</div>
            <div class="mt-2 space-y-0.5 text-[0.8rem] text-gray-600">
      ...
      <div
        class="rounded-lg border border-gray-200 bg-gray-50 px-2.5 py-2 text-[0.8rem] text-gray-600"
      >
